...
const NewsApp = () => {
  ...
  const API_KEY = '4dddf6e0f8364b8cb341f5d787dacec8E'; // Replace with your API key

  const getData = async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await fetch(`https://newsapi.org/v2/everything?q=${search}&from=2024-07-21&sortBy=publishedAt&apiKey=${API_KEY}`);
